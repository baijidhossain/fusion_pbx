...
if (is_array($dashboard) && @sizeof($dashboard) != 0) {
  $expanded_all = true;
  foreach ($dashboard as $row) {
    if ($row['dashboard_details_state'] == 'contracted' || $row['dashboard_details_state'] == 'hidden') {
      $expanded_all = false;
    }
  }
}
...
foreach ($dashboard as $row) {
  ...
  echo '<pre>';
  print_r($row['dashboard_path']);
  echo '</pre>';

  echo '<pre>';
  print_r($_SESSION['domain_uuid']);
  echo '</pre>';
...
?>
  ...
  <style>
    ...
    .ghost {
      ...
      <?php $br = format_border_radius($_SESSION['theme']['dashboard_border_radius']['text'] ?? null, '5px'); ?>-webkit-border-radius: <?php echo $br['tl']['n'] . $br['tl']['u']; ?> <?php echo $br['tr']['n'] . $br['tr']['u']; ?> <?php echo $br['br']['n'] . $br['br']['u']; ?> <?php echo $br['bl']['n'] . $br['bl']['u']; ?>;
